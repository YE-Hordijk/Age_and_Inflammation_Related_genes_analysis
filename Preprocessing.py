...
#class Settings():
#	Tissue = "Whole Blood" #"Bladder" #"Whole Blood"

#update = False #False
#make_files_for_R = True
...
help_name = GENE_SELECTION #variable for nameing files and folders
...
def strip_data_file(filename, lambda_function, header, separator):
	...
	if (lambda_function == 0):
		data = pd.read_csv(filename, header=header, usecols=["SAMPID","SMTSD"], sep=separator).query('SMTSD == "{}"'.format(tissue)) #Change the sample type here
	else:
...
def make_metadata(RNAseq, subjects):
	temp_subjects = df_subjects["Age".upper()].to_dict() #making dictionary where SubjectID is key and Age is value
	...
	for column in RNAseq:
		if ((column != "Name") and (column != "Description")):
			...
			elif use_middle_age:								metadata[column] = "Middle (50-59)"
	return metadata
	"""
	for column in RNAseq:
		if ((column != "Name") and (column != "Description")):
			...
			elif use_middle_age:								metadata[column] = "Middle ("+P.MIDDLE+")"
...
meta = make_metadata(df_RNA_seq, df_subjects) #Create a dictionary with sample ids as keys and age groups as values
...
if select_on_genes: help_name = help_name[:-12] + "unnormalized.txt" #Changing the help_name for naming files and folders
else: 
	if use_middle_age: tempvar = "_With-MiddleAge"
