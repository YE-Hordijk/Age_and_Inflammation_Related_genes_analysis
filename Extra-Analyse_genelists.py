...
exp_type2 = ""
exp_type = input("TotalGeneDataset[1] or Compare_outliers[2]?")#" or Combine_End_Result?[3]")
if exp_type == "1": 
...
for GENEDATASET in lijst: 
	...
	if exp_type2 == "2":
		...
		if exp_type != "3":
			...
			for genelist in listoffiles:
				...
				for i in f: 
					if res == "[DecisionTree]": 
						if genelist[14:18] == "_Old": DT_list[1].append(i.split(" ")[0]) # Old
						else: 												DT_list[0].append(i.split(" ")[0]) # Young

					elif res == "[RandomForest]": 
						if genelist[14:18] == "_Old": RF_list[1].append(i.split(" ")[0]) # Old
						else: 												RF_list[0].append(i.split(" ")[0]) # Young
						
					elif res == "[Support Vector Machine]": 
						if genelist[14:18] == "_Old": SVM_list[1].append(i.split(" ")[0]) # Old
						else: 												SVM_list[0].append(i.split(" ")[0]) # Young
			...
			if exp_type2 == '1' and use_only_overlap: # for ever model there are 2 lists (yound and old)
				DT_list = set(DT_list[0]) & set(DT_list[1])
				RF_list = set(RF_list[0]) & set(RF_list[1])
				SVM_list = set(SVM_list[0]) & set(SVM_list[1])
			else:
				DT_list = DT_list[0] + DT_list[1]
				RF_list = RF_list[0] + RF_list[1]
				SVM_list = SVM_list[0] + SVM_list[1]
	...
	plt.savefig(P.experiment_name+'/VennDiagrams/'+foldername+'/'+GENEDATASET+'.png', bbox_inches = 'tight')
	#plt.show()
	plt.close()
